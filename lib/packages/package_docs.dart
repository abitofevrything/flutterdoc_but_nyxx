...
final Logger _logger = Logger('FLutterDoc.Packages');
...
class PackageDocs {
  ...
  String get urlToDocs => 'https://pub.dev/documentation/$packageName/latest/';
  ...
  static Future<PackageDocs> fromPackage(String packageName) async {
    final docs = PackageDocs(packageName: packageName);
    await docs.update();
    return docs;
  }
  ...
  Future<void> update() async {
    ...
    final response = await http.get(Uri.parse(urlToDocs).resolve('index.json'));

    if (response.statusCode != 200) {
      _logger
          .shout('Unable to update docs for package "$packageName": Error ${response.statusCode}');
      return;
    }
    ...
    try {
      ...
      for (final dataEntry in data) {
        final entry = DocEntry.fromJson(dataEntry, this);
...
class DocEntry {
  ...
  factory DocEntry.fromJson(Map<String, dynamic> json, PackageDocs docs) {
    String displayName;
    ...
    return DocEntry(
      ...
      urlToDocs: Uri.parse(docs.urlToDocs).resolve(json['href'] as String).toString(),
    );
  }
}
